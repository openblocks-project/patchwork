...
/// OB Distance — Time-of-Flight distance sensor (VL53L0X over I2C) plus an
...
/// and bound by `("distance", id)`. No port to choose; manual override under
/// "Port (manual)" for non-standard boards.
///
/// Hardware: VL53L0X on I2C (SDA→GPIO8, SCL→GPIO9), WS2812 strip DATA→A2.
///
/// Protocol:
///   device → Patchwork: `/sys/ready distance <id>`, `/distance/<id>/mm <millimeters>`
///   Patchwork → device: `/distance/<id>/color <r> <g> <b>`, `/distance/<id>/blink`
pub fn render(
    ui: &mut egui::Ui,
    node_id: NodeId,
    ...
    let (device_id, label_color, port_name, selected_port) = match node_type {
        NodeType::ObDistance { device_id, label_color, port_name, selected_port, .. } =>
            (device_id, label_color, port_name, selected_port),
        _ => return,
    };
    let did = *device_id;
    let error_id = egui::Id::new(("ob_dist_error", node_id));
    let led_sent_key = egui::Id::new(("ob_dist_led_sent", node_id));

    // Snapshot device state up front (owned).
    let (present, active, val, mm) = match ob_manager.device("distance", did) {
        Some(d) => (
            true, d.is_active,
    ...
    ui.collapsing("Port (manual)", |ui| {
        ...
        ui.horizontal(|ui| {
            ui.label("Port:");
            egui::ComboBox::from_id_salt(egui::Id::new(("ob_dist_port", node_id)))
    ...
    if present {
        ...
        if last_sent.is_none() {
            ...
            ob_manager.send_to_device("distance", did, &format!("/distance/{}/blink", did));
        }
        if last_sent != Some(*label_color) {
            ob_manager.send_to_device("distance", did, &format!("/distance/{}/color {} {} {}",
    ...
    // ── Horizontal bar of the normalized value ──────────────────────────────
    ...
    // ── Readouts: normalized value + raw mm ─────────────────────────────────
    ui.horizontal(|ui| {
