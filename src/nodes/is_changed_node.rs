...
impl NodeBehavior for IsChangedNode {
    ...
    fn evaluate(&mut self, inputs: &[PortValue]) -> Vec<(usize, PortValue)> {
        ...
        // Unwired ports (PortValue::None) must NOT be coerced to the type's
        // zero value (0.0 / "" / no-image) — that fires a phantom "change"
        // when the user disconnects a wire. Drop `initialized` so the first
        // wired sample after a (re)connect re-seeds the baseline.
        if matches!(input, PortValue::None) {
            self.initialized = false;
            let value_out = match self.mode {
                IsChangedMode::Number => PortValue::Float(self.previous_number),
                IsChangedMode::Text   => PortValue::Text(self.previous_text.clone()),
                IsChangedMode::Image  => self.previous_image.clone()
                    .map_or(PortValue::None, PortValue::Image),
            };
            return vec![(0, PortValue::Float(0.0)), (1, value_out)];
        }
