...
impl VideoSource {
    ...
    /// Whether this source is wired up in the current build. Phase 1
    /// added Screen. Phase 2 M5 adds Syphon on macOS. NDI / File /
                ...
                .show_ui(ui, |ui| {
                    for s in VideoSource::all() {
                        ...
                        ui.add_enabled_ui(s.available(), |ui| {
                            ui.selectable_value(&mut self.source, s, text);
                        });
                    }
                });
...
impl VideoInNode {
    fn render_camera_ui(&mut self, ui: &mut egui::Ui, ctx: &mut RenderContext) {
        ...
        if let Some(idx) = new_idx {
            ...
            if was_active { self.open_decoder(); }
        }
        ...
        if toggle_start { self.open_decoder(); }
